...
4.31; that complex is now built and proved exact by hand in `Koszul.lean`,
and `KoszulHomology.lean` transports its top homology along the syzygies:
`socleEquivH₀OfResolution` gives `Soc M ≅ F₃ ⧸ m F₃` for any minimal
length-three free resolution. What is left is to instantiate that at the
complex `GradedMinimalFreeComplex.ofLevel hA` and to identify `F₃ ⧸ m F₃`
with `k^{β₃}`. Everything else — the shift `e+3`, full support of the last

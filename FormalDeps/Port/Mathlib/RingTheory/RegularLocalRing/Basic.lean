...
lemma IsLocalRing.ResidueField.map_bijective_of_surjective [IsLocalRing R] {S : Type*} [CommRing S]
  ...
  unfold ResidueField.map
  apply Ideal.Quotient.lift_surjective_of_surjective
  exact (Ideal.Quotient.mk_surjective (I := maximalIdeal S)).comp surj
  ...
  tfae_have 2 → 3 := by
    ...
    have ge : (Submodule.spanFinrank (maximalIdeal (R ⧸ Ideal.span (S : Set R)))) + S.card ≤
      ...
      have rk := rank_eq_of_equiv_equiv
        ...
        e (IsLocalRing.ResidueField.map_bijective_of_surjective _
          Ideal.Quotient.mk_surjective) (fun r m ↦ by
    ...
    exact ⟨ENat.WithBot.add_natCast_cancel.mp
  ...
  · rename_i n ih _ _
    obtain ⟨x, xmem, xnmem⟩ : ∃ x ∈ maximalIdeal R,
      ...
      · let _ : I.IsPrime := Ideal.minimalPrimes_isPrime min
        rw [← Ideal.height_eq_ringKrullDim_iff.mpr (le_antisymm (le_maximalIdeal_of_isPrime I)
          sub), Ideal.height_eq_zero_iff.mpr min, ← Nat.cast_zero] at hn
    ...
    have ih' := ih (R ⧸ Ideal.span {x}) (ENat.WithBot.add_natCast_cancel.mp dim)
...
theorem isRegular_of_span_eq_maximalIdeal [IsRegularLocalRing R] (rs : List R)
  ...
  have : IsDomain (R ⧸ Ideal.ofList (List.take i rs)) := by
    ...
    have hset : ((List.take i rs).toFinset : Set R) = {r | r ∈ List.take i rs} := by
      ext r
      simp
    have hI : Ideal.span ((List.take i rs).toFinset : Set R) =
        Ideal.ofList (List.take i rs) := by
      rw [Ideal.ofList, hset]
    letI : IsRegularLocalRing
        (R ⧸ Ideal.span ((List.take i rs).toFinset : Set R)) := reg.1
    exact IsRegularLocalRing.of_ringEquiv
      (Ideal.quotientEquivAlgOfEq R hI).toRingEquiv

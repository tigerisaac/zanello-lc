...
public import FormalDeps.Port.Mathlib.RingTheory.RegularLocalRing.GlobalDimension
...
theorem IsRegularRing.globalDimension_eq_ringKrullDim [Small.{v} R] [IsRegularRing R]
  ...
  · rw [globalDimension_eq_iSup_loclization_maximal]
    ...
    have bdd : BddAbove (Set.range f) := by
      ...
      simpa [← hy, WithBot.le_unbot_iff, f] using
        Ideal.height_le_ringKrullDim_of_ne_top y.2.ne_top
    rw [← Ideal.sup_isMaximal_height_eq_ringKrullDim, iSup_subtype', WithBot.coe_iSup bdd]
    apply le_antisymm
    · simp only [iSup_le_iff]
      intro p
      have _ : p.1.IsPrime := Ideal.IsMaximal.isPrime' p.1
      let _ : IsRegularLocalRing (Localization.AtPrime p.1) :=
        isRegularRing_iff.mp ‹_› p.1
    ...
    · simp only [iSup_le_iff]
      intro ⟨p, hp⟩
      have _ : p.IsPrime := Ideal.IsMaximal.isPrime' p
      let _ : IsRegularLocalRing (Localization.AtPrime p) :=
        isRegularRing_iff.mp ‹_› p
      let _ : Small.{v} (Localization.AtPrime p) :=
        small_of_surjective Localization.mkHom_surjective
      simp only [ge_iff_le, f,

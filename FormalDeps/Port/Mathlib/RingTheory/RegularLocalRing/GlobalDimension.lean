...
public import FormalDeps.Port.Mathlib.RingTheory.CohenMacaulay.Maximal
public import FormalDeps.Port.Mathlib.RingTheory.GlobalDimension
public import FormalDeps.Port.Mathlib.RingTheory.Regular.AuslanderBuchsbaum
  ...
  obtain ⟨n, hn⟩ := finite_projectiveDimension_of_isRegularLocalRing_aux M m
    (by
      ...
      simpa [add_comm] using
        (WithBot.le_self_add (WithBot.coe_ne_bot :
          (↑(IsLocalRing.depth M) : WithBot ℕ∞) ≠ ⊥) (m : WithBot ℕ∞)))
...
theorem IsRegularLocalRing.globalDimension_eq_ringKrullDim [Small.{v} R] [IsRegularLocalRing R] :
  ...
  · let _ : Small.{v, u} (ResidueField R) := small_of_surjective IsLocalRing.residue_surjective
    ...
    have hle : projectiveDimension k ≤
        ⨆ M : ModuleCat.{v} R, ⨆ (_ : Module.Finite R M), projectiveDimension M :=
      le_iSup_of_le k (le_iSup_of_le (inferInstance : Module.Finite R k) le_rfl)
    simpa [← eq, eq0] using hle

...
public import FormalDeps.Port.Mathlib.RingTheory.Regular.Depth
  ...
  · have eqr (n : ℕ∞) : n.toNat = r → n = r := by
    ...
    · intro L _ _ _ p e Lntr dim_eq
        ...
        have dimlt' : (Module.supportDim R (QuotSMulTop x L)).unbot
          ...
          have : (Module.supportDim R (QuotSMulTop x L)) + 1 ≤ Module.supportDim R L := by
            ...
            have ple : p.asIdeal ≤ Module.annihilator R (QuotSMulTop x L) := by
              rw [← Ideal.annihilator_quotient (I := p.asIdeal),
                ← LinearEquiv.annihilator_eq e]
              exact LinearMap.annihilator_le_of_surjective
                (x • (⊤ : Submodule R L)).mkQ (Submodule.mkQ_surjective _)
      ...
      have epi' : Function.Surjective
        ...
        have surj := (AddCommGrpCat.epi_iff_surjective _).mp <| ShortComplex.Exact.epi_f
          (Ext.contravariant_sequence_exact₁' hS M i (i + 1) (Nat.add_comm 1 i))
          (zero.eq_zero_of_tgt _)
        have hfun : ⇑(x • LinearMap.id (R := R) (M := (Ext (of R L) M i))) =
            ⇑(AddCommGrpCat.ofHom
              ((Ext.mk₀ ((of R L).smulShortComplex x).f).precomp M (zero_add i))) := by
          funext a
          change x • a = (Ext.mk₀ ((of R L).smulShortComplex x).f).comp a (zero_add i)
          simp only [ModuleCat.smulShortComplex]
          have hf : (ModuleCat.ofHom ((LinearMap.lsmul R L) x) :
              ModuleCat.of R L ⟶ ModuleCat.of R L) = x • 𝟙 (ModuleCat.of R L) :=
            ModuleCat.lsmul_eq_smul_id (ModuleCat.of R L) x
          rw [hf, Ext.mk₀_smul, Ext.smul_comp, Ext.mk₀_id_comp]
        rw [hfun]
        exact surj
...
theorem depth_le_ringKrullDim_associatedPrime [IsNoetherianRing R] [IsLocalRing R]
  ...
  have hdim : Module.supportDim R (Shrink.{v} (R ⧸ P)) = ringKrullDim (R ⧸ P) :=
    (Module.supportDim_eq_of_equiv (Shrink.linearEquiv R (R ⧸ P))).trans
      (Module.supportDim_quotient_eq_ringKrullDim P)
  have hunbot := (WithBot.unbot_inj
    (Module.supportDim_ne_bot_of_nontrivial R (Shrink.{v} (R ⧸ P)))
    (quotient_prime_ringKrullDim_ne_bot ass.1)).mpr hdim
  exact hunbot ▸ this

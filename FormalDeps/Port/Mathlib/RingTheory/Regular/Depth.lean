...
public import FormalDeps.Port.Mathlib.RingTheory.Regular.Category
...
lemma mono_of_mono (a : R) {k : ℕ} (kpos : k > 0) (i : ℕ) {M N : ModuleCat.{v} R}
  ...
  · rename_i k ih
    ...
    · have eq_comp : (AddCommGrpCat.ofHom ((Ext.mk₀ (smulShortComplex M (a ^ k * a)).f).postcomp
        ...
        N (add_zero i))) := by
        change (extFunctorObj N i).map (smulShortComplex M (a ^ k * a)).f =
          (extFunctorObj N i).map (smulShortComplex M (a ^ k)).f ≫
            (extFunctorObj N i).map (smulShortComplex M a).f
        rw [← (extFunctorObj N i).map_comp]
        congr 1
        ext m
        change (a ^ k * a) • m = a • (a ^ k • m)
  ...
  · rename_i n hn _ _ _
    ...
    | x :: rs' =>
      ...
      have eq : RingHom.ker f = ofList (x :: rs') := by
        ...
        simp only [smul_eq_mul, mul_top] at this
        dsimp only [f]
        rw [← RingHom.comap_ker, Ideal.mk_ker, ← Ideal.map_ofList,
          Ideal.comap_map_of_surjective' (Ideal.Quotient.mk (x • (⊤ : Ideal R)))
            Ideal.Quotient.mk_surjective, Ideal.mk_ker, ofList_cons, this, sup_comm]

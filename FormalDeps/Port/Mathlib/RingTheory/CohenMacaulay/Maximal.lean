...
public import FormalDeps.Port.Mathlib.RingTheory.CohenMacaulay.Basic
public import FormalDeps.Port.Mathlib.RingTheory.RegularLocalRing.Basic
...
lemma free_of_quotSMulTop_free [IsLocalRing R] [IsNoetherianRing R] (M : Type*) [AddCommGroup M]
  ...
  have surjf : Function.Surjective f := by
    dsimp only [f, LinearMap.coe_comp, Function.comp_apply]
    apply b'.symm.surjective.comp
    intro y
    obtain ⟨z, hz⟩ := Finsupp.mapRange_surjective _ rfl
      (Submodule.mkQ_surjective (Ideal.span {x})) y
    use z
    ext i
  ...
  · rename_i n ih _ _ _ _ _
    ...
    · obtain ⟨x, xmem, xnmem⟩ : ∃ x ∈ maximalIdeal R, x ∉ (maximalIdeal R) ^ 2 := by
      ...
      have dim : ringKrullDim (R ⧸ Ideal.span {x}) = n := by
        ...
        exact ENat.WithBot.add_natCast_cancel.mp this

...
public import FormalDeps.Port.Mathlib.RingTheory.Regular.Ischebeck
...
lemma associated_prime_minimal_of_isCohenMacaulay (M : ModuleCat.{v} R)
  ...
  · classical
    ...
    have netop : (ringKrullDim (R ⧸ p)).unbot nebot ≠ ⊤ := by
      have : FiniteRingKrullDim R := by infer_instance
...
lemma isLocalizedModule_quotSMulTop_isLocalizedModule_map (x : R)
  ...
  exists_of_eq {y1 y2} h := by
    ...
    have h : (0 : QuotSMulTop x M) = Submodule.Quotient.mk (c • s • (y1 - y2)) := by
      ...
      rw [hc]
      rw [Submonoid.smul_def, smul_comm (c : R) x z]
      exact Submodule.smul_mem_pointwise_smul (c • z) x ⊤ Submodule.mem_top
...
lemma isLocalize_at_prime_dim_eq_prime_depth_of_isCohenMacaulay
  ...
  · simp only [← hn, CharP.cast_eq_zero, WithBot.coe_zero]
    have min : p ∈ (Module.annihilator R M).minimalPrimes := by
      ...
      have hp'' : LinearMap.ker (LinearMap.toSpanSingleton R M (g 1)) ≤ p' := by
        simpa only [SetLike.le_def, LinearMap.mem_ker, LinearMap.toSpanSingleton_apply,
          Submodule.mem_colon_singleton, Submodule.mem_bot] using hp'
...
lemma isCohenMacaulayRing_iff [IsNoetherianRing R] : IsCohenMacaulayRing R ↔
  ...
  have disj' : Disjoint (m.primeCompl : Set R) (p : Set R) := by
    simpa [Ideal.primeCompl] using disj
  have : (p.map (algebraMap R Rₘ)).IsPrime := by
    rw [IsLocalization.isPrime_iff_isPrime_disjoint m.primeCompl Rₘ,
      IsLocalization.under_map_of_isPrime_disjoint m.primeCompl Rₘ hp disj']
    exact ⟨hp, disj'⟩
...
lemma IsCohenMacaulayRing.of_isCohenMacaulayLocalRing [IsCohenMacaulayLocalRing R]
  ...
  let _ := IsLocalization.of_le_isUnit (S := m.primeCompl)
    (fun x hx ↦ by
      change IsUnit x
      simpa [Ideal.primeCompl, eq_maximalIdeal hm, IsLocalRing.mem_maximalIdeal] using hx)
...
lemma IsCohenMacaulayLocalRing.of_isLocalRing_of_isCohenMacaulayRing [IsLocalRing R]
  ...
  let _ := IsLocalization.of_le_isUnit (S := (maximalIdeal R).primeCompl)
    (fun x hx ↦ by
      change IsUnit x
      simpa [Ideal.primeCompl, IsLocalRing.mem_maximalIdeal] using hx)
...
lemma quotient_regular_sequence_isCohenMacaulay_iff_isCohenMacaulay [IsLocalRing R]
  ...
  have reg' : IsRegular R rs :=
    ⟨reg, by
      intro h
      have heq : (⊤ : Ideal R) = Ideal.ofList rs := by
        ext x
        constructor
        · intro _
          have hx : x ∈ (⊤ : Submodule R R) := Submodule.mem_top
          rw [h] at hx
          simpa using hx
        · exact fun _ ↦ Submodule.mem_top
      exact ((span_le.mpr mem).trans_lt IsPrime.ne_top'.lt_top).ne_top.symm heq⟩
  simp only [isCohenMacaulayLocalRing_def,

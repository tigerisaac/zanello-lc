...
lemma ext_quotient_one_subsingleton_iff [Small.{v} R] (M : ModuleCat.{v} R) (I : Ideal R) :
  ...
  have exac : Function.Exact Sf Sg := by
    ...
    constructor
    · intro hx
      have hxmem : (equivShrink R).symm x ∈ I := by
        rw [← Ideal.Quotient.eq_zero_iff_mem]
        have hx' := congrArg (equivShrink (R ⧸ I)).symm hx
        simpa [Sg] using hx'
      refine ⟨(equivShrink I) ⟨(equivShrink R).symm x, hxmem⟩, ?_⟩
      simp [Sf]
    · rintro ⟨y, rfl⟩
      have hq : Ideal.Quotient.mk I (↑((equivShrink I).symm y) : R) = 0 :=
        Ideal.Quotient.eq_zero_iff_mem.mpr ((equivShrink I).symm y).2
      simp [Sf, Sg, hq]
      exact (Shrink.linearEquiv R (R ⧸ I)).symm.map_zero
